from predict.graph import import_tf
import predict.modeling
import predict.tokenization
from predict.graph import optimize_graph
import predict.args
...
class BertVector:
    ...
    def __init__(self, batch_size=32, pooling_strategy="REDUCE_MEAN", max_seq_len=40):
        ...
        self.layer_indexes = predict.args.layer_indexes
        self.gpu_memory_fraction = 1

        if pooling_strategy == "NONE":
            pooling_strategy = predict.args.PoolingStrategy.NONE
        elif pooling_strategy == "REDUCE_MAX":
            pooling_strategy = predict.args.PoolingStrategy.REDUCE_MAX
        elif pooling_strategy == "REDUCE_MEAN":
            pooling_strategy = predict.args.PoolingStrategy.REDUCE_MEAN
        elif pooling_strategy == "REDUCE_MEAN_MAX":
            pooling_strategy = predict.args.PoolingStrategy.REDUCE_MEAN_MAX

        self.graph_path = optimize_graph(pooling_strategy=pooling_strategy, max_seq_len=self.max_seq_length)

        self.tokenizer = predict.tokenization.FullTokenizer(vocab_file=predict.args.vocab_file, do_lower_case=True)
    ...
    def model_fn_builder(self, bert_config, init_checkpoint, layer_indexes):
        ...
        def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
            ...
            with jit_scope():
                model = predict.modeling.BertModel(
                    config=bert_config,
                    is_training=False,
                    input_ids=input_ids,
                ...
                (assignment_map, initialized_variable_names) = predict.modeling.get_assignment_map_from_checkpoint(tvars,
                                                                                                           init_checkpoint)
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            if ex_index < 5:
                ...
                tf.logging.info("tokens: %s" % " ".join(
                    [predict.tokenization.printable_text(x) for x in tokens]))
    ...
    def _to_example(sentences):
        ...
        unique_id = 0
        for ss in sentences:
            line = predict.tokenization.convert_to_unicode(ss)
            if not line:
                continue
            line = line.strip()
            text_a = None
