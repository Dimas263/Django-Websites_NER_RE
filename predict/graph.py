...
import predict.args
from predict.args import PoolingStrategy
import contextlib

import torch
from torch import cuda

device = '0' if cuda.is_available() else '-1' # 0 cuda, -1 cpu

def import_tf(device_id=device, verbose=False):
...
def optimize_graph(logger=None, verbose=False, pooling_strategy=PoolingStrategy.REDUCE_MEAN, max_seq_len=40):
    ...
    try:
        ...
        config_fp = predict.args.config_name
        init_checkpoint = predict.args.ckpt_name
        logger.info('model config: %s' % config_fp)

        # Load the bert configuration file
        with tf.gfile.GFile(config_fp, 'r') as f:
            bert_config = predict.modeling.BertConfig.from_dict(json.load(f))
        ...
        # xla acceleration
        jit_scope = tf.contrib.compiler.jit.experimental_jit_scope if predict.args.xla else contextlib.suppress
        ...
        with jit_scope():
            ...
            with tf.variable_scope("pooling"):
                ...
                if len(predict.args.layer_indexes) == 1:
                    encoder_layer = model.all_encoder_layers[predict.args.layer_indexes[0]]
                else:
                    ...
                    all_layers = [model.all_encoder_layers[l] for l in predict.args.layer_indexes]
